...
while [[ $count_message -gt 0 ]]; do
    # Display message with carriage return (\r) to overwrite the same line
    echo -ne "\r${CYAN}This screen will clear in${NC} ${BLUE}$count_message${NC} ${CYAN}second[s].${NC} Press any key to skip... "

    # Wait for 1 second (-t 1) for a single keypress (-n 1)
    # -s hides the typed character (optional)
    if read -t 1 -n 1 -s key; then
    # If read returns 0, a key was pressed. Break the loop.
        break
        clear
    fi
...
# Ensure screen is cleared if countdown completed without keypress
clear
...
apt autoclean -y -q && apt autoremove -y -q # Clean up diskspace
...
GRUB_EOF
    ...
    if ! command -v convert &> /dev/null; then
        ...
        apt install imagemagick -y -q 2>/dev/null || {
    ...
    if command -v convert &> /dev/null; then
        ...
        # Compatible background with most systems (use as fallback if running into issues)
...
EOF
    ...
    echo -e "${CYAN}💿 ISO: $BUILDF/$LIVE_BOOT/$DE/debian13-$DE-x64-livecd.iso${NC}\n"
    exit 1
}
...
EOF
    ...
    trap 'umount /proc 2>/dev/null; umount /sys 2>/dev/null; umount /dev/pts 2>/dev/null; umount /dev 2>/dev/null; unset USER1 PASS1; exit' ERR EXIT
...
install_plymouth() {
    # Install plymouth
    DEBIAN_FRONTEND=noninteractive apt install plymouth plymouth-themes -y -q
...
SKEL_EOF
    ...
    # Install & configure required nerd fonts for terminal icons support (uncomment to use)
    # These fonts provide wide support for most terminals
    #apt install fontconfig -y -q
    #cd /tmp
...
SERVICE_EOF
    ...
    # Final cleanup
    apt autoclean -y -q && apt autoremove -y -q
...
unset PASS1 USER1 CHRUSER1 HASHP
